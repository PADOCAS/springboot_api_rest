...
public class ControleExcecoes {
    ...
    protected ResponseEntity<Object> handleExceptionDataIntegry(Exception ex, WebRequest request) {
        ...
        String requestUri = ((ServletWebRequest) request).getRequest().getRequestURI();
        ObjetoErro objetoErro = new ObjetoErro(ex.getClass().getCanonicalName(), msg.toString(), requestUri, HttpStatus.INTERNAL_SERVER_ERROR.value() + " ==> " + HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), LocalDateTime.now());
